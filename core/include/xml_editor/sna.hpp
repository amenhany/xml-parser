...
namespace xml_editor::sna {
    ...
    std::vector<const Post*> search_by_word(const Graph& xmlGraph, const std::string& word);
    std::vector<const Post*> search_by_topic(const Graph& xmlGraph, const std::string& topic);
