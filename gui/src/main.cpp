...
// #include "xml_editor/gui_runner.hpp" // Removed - file doesn't exist
...
int run_gui(int argc, char *argv[]) {
  ...
  QObject::connect(browseButton, &QPushButton::clicked, [&]() {
    QString fileName = QFileDialog::getOpenFileName(
        &window, "Select File", "", "XML Files (*.xml);;All Files (*)");
    if (!fileName.isEmpty()) {
  ...
  QObject::connect(minifyButton, &QPushButton::clicked, [&]() {
    ...
    QString minified =
        QString::fromStdString(xml_editor::xml::minify(xmlText.toStdString()));
  ...
  QObject::connect(compressButton, &QPushButton::clicked, [&]() {
    ...
    QString compressed = QString::fromStdString(
        xml_editor::xml::compress(xml_editor::xml::minify(xmlText.toStdString())));
  ...
  QObject::connect(decompressButton, &QPushButton::clicked, [&]() {
    ...
    try {
      QString decompressed = QString::fromStdString(
          xml_editor::xml::decompress(xmlText.toStdString()));
      outputArea->setText(decompressed);
    } catch (const std::exception &e) {
      QMessageBox::warning(&window, "Decompression Error", e.what());
    } catch (...) {
      QMessageBox::warning(&window, "Decompression Error",
  ...
  QObject::connect(saveButton, &QPushButton::clicked, [&]() {
    QString saveFile = QFileDialog::getSaveFileName(
        &window, "Save Output", "", "Text Files (*.txt);;All Files (*)");
    if (!saveFile.isEmpty()) {
      QFile file(saveFile);
